...
func (s *MPoolStats) RecordAlloc(tag string, sz int64) int64 {
	...
	if curr > hwm {
		...
		if swapped && curr/GB != hwm/GB {
			log.Printf("MPool %s new high watermark\n%s", tag, s.Report("    "))
		}
	}
	return curr
}
...
func (s *MPoolStats) RecordFree(tag string, sz int64) int64 {
	...
	curr := s.NumCurrBytes.Add(-sz)
	if curr < 0 {
		// Todo replace with uber log
		log.Printf("Mpool %s free bug, stats: %s", tag, s.Report("    "))
		panic("mpool freed more bytes than alloc")
	}
	return curr
}
...
func (s *MPoolStats) RecordManyFrees(tag string, nfree, sz int64) int64 {
	...
	curr := s.NumCurrBytes.Add(-sz)
	if curr < 0 {
		log.Printf("Mpool %s free many bug, stats: %s", tag, s.Report("    "))
		panic("mpool freemany freed more bytes than alloc")
	}
	return curr
}
...
func (fp *fixedPool) free(hdr *memHdr) {
	if hdr.poolId != fp.poolId || hdr.fixedPoolIdx != fp.fpIdx ||
		...
		!hdr.CheckGuard() {
		panic("mpool fixed pool hdr corruption.   Possible double free")
	}
...
func (mp *MPool) destroy() {
	if mp.stats.NumAlloc.Load() < mp.stats.NumFree.Load() {
		// Todo replace with uber log
		log.Printf("mp error: %s", mp.stats.Report(""))
	}
...
func NewMPool(tag string, cap int64, flag int) (*MPool, error) {
	if cap > 0 {
		// simple sanity check
		if cap < 1024*1024 {
			return nil, fmt.Errorf("mpool cap %d too small", cap)
		}
		if cap > GlobalCap() {
			return nil, fmt.Errorf("mpool cap %d too big, global cap %d", cap, globalCap)
		}
	}
...
func (mp *MPool) Alloc(sz int) ([]byte, error) {
	if sz < 0 || sz > GB {
		return nil, fmt.Errorf("Invalid alloc size %d", sz)
	}
	...
	if gcurr > GlobalCap() {
		...
		return nil, errors.New("error: out of memory")
	}
	...
	if mycurr > mp.Cap() {
		...
		return nil, fmt.Errorf("mpool out of space, alloc %d bytes, cap %d", sz, mp.cap)
	}
...
func (mp *MPool) Free(bs []byte) {
	...
	if !pHdr.CheckGuard() {
		panic("mp header corruption")
	}

	if pHdr.poolId == mp.id {
		if pHdr.allocSz == -1 {
			// double free.
			panic("free size -1, possible double free")
		}
		...
		if !ok {
			log.Fatalf("invalid mpool id %d", pHdr.poolId)
		}
...
func (mp *MPool) Grow(old []byte, sz int) ([]byte, error) {
	if sz < len(old) {
		return nil, fmt.Errorf("mpool grow actually shrinks, %d, %d", len(old), sz)
	}
	if sz <= cap(old) {
		return old[:sz], nil
	}
...
func (mp *MPool) Grow2(old []byte, old2 []byte, sz int) ([]byte, error) {
	...
	len2 := len(old2)
	if sz < len1+len2 {
		return nil, fmt.Errorf("mpool grow2 actually shrinks, %d+%d, %d", len1, len2, sz)
	}
...
func (mp *MPool) Increase(nb int64) error {
	...
	if mycurr > mp.Cap() {
		...
		return fmt.Errorf("mpool out of space, alloc %d bytes, cap %d", nb, mp.cap)
	}
	return nil
}
